...
class Customer:
    ...
    def __init__(self, x, y, step, infected=0, probSpreadPlume=PROBSPREADPLUME):
    ...
    def updateFirstTarget(self, store):
        ...
        for i in range(0, len(self.shoppingList)):
            ...
            thisDist = store.staticGraph.shortest_paths_dijkstra(
                source=startInd, target=targetInd, weights=None)[0][0]
    ...
    def initShoppingList(self, store, maxN):
        ...
        while len(self.shoppingList) < targetsDrawn:
            ...
            # running from 1 to avoid customers visiting the row of cashiers before leaving
            ty = np.random.randint(1, store.Ly)
            # last ensures that customers avoid exits before leaving
            while store.blocked[tx, ty] or [tx, ty] in self.shoppingList or ([tx, ty] in store.exit) or (store.entrance[0] == tx and store.entrance[1] == ty) or (tx < 1 or ty < 1) or (tx < 3 and ty < 3) or not (store.blockedShelves[tx, ty-1] or (ty+1 < store.Ly and store.blockedShelves[tx, ty+1]) or store.blockedShelves[tx-1, ty] or (tx+1 < store.Lx and store.blockedShelves[tx+1, ty])):
                tx = np.random.randint(store.Lx)
    ...
    def takeStep(self, store):
        ...
        if store.plumes[self.x, self.y] and not store.useDiffusion:
            ...
            if not self.infected:
                store.storeWideExposure += store.plumes[self.x,
                                                        self.y]*store.dt
        ...
        if not len(self.shoppingList):
            ...
            # shopping list done, exit found
                ...
                # make sure exit is not blocked once customer leaves the simulation
                store.blocked[self.x, self.y] = 0
                return -1, -1
        ...
        if self.path is None or not len(self.path):
            ...
            self.path = store.staticGraph.get_shortest_paths(
                startInd, to=targetInd)[0]
            self.path.pop(0)
