...
class Simulation:
    def __init__(self, gui, seed, Lx, Ly, nShelves, nCustomers=1, probNewCustomer=0.1, probInfCustomer=0.05, nPlumes=20, maxSteps=1000, outputLevel=0, importGeometry=1, useDiffusion=0, imageName="test.pbm", dx=1.0):
        ...
        # sets the output level: if 1, plots figures (slow). if 0, only customer data output written
    ...
    def newCustomer(self):
        ...
        if np.random.rand() < self.probInfCustomer:
            ...
            infected = 0
        new = SmartCustomer(
            self.store.entrance[0], self.store.entrance[1], step=self.stepNow, infected=infected)
    ...
    def printStore(self, step, title):
        ...
        plt.axis('off')
        plt.savefig(
            "simFigures/simFigure_{}_{:07d}.png".format(self.seed, step))
        plt.close()
        return
    ...
    def printEndStatistics(self):
        ...
        header = "seed {}, storedims {}x{}, probNewCustomer {}, probInfCustomer {}, steps {}, avrCustomersInStore {}, NEXITS {}, PROBSPREADPLUME {}".format(
            self.seed, self.store.Lx, self.store.Ly, self.probNewCustomer, self.probInfCustomer, self.maxSteps, np.mean(self.customersNowInStore), NEXITS, PROBSPREADPLUME)
        if not self.useDiffusion:
            header += " PLUMELIFETIME {}".format(PLUMELIFETIME)
        else:
            header += " PLUMECONCINC {}, DIFFCOEFF {}, SINKCOEFF {}".format(
                PLUMECONCINC, DIFFCOEFF, ACSINKCOEFF)
        # data per customer
        dataArr = np.array([self.customerInfected[:self.customerNow], self.itemsBought[:self.customerNow], self.exposureHist[:self.customerNow],
                            self.timeSpent[:self.customerNow], self.exposureHistTime[:self.customerNow], self.exposureHistTimeThres[:self.customerNow]]).T
        np.savetxt("customer_data_{}.dat".format(
            self.seed), dataArr, header=header)
        # store wide data of number of customer and customers in queue
        dataArr = np.array([self.customersNowInStore[:self.stepNow], self.customersNowInQueue[:self.stepNow],
                            self.emittingCustomersNowInStore[:self.stepNow], self.exposureDuringTimeStep[:self.stepNow]]).T
        np.savetxt("store_data_{}.dat".format(
            self.seed), dataArr, header=header)
        np.savetxt("integrated_plumes_store_data_{}.dat".format(
            self.seed), self.store.plumesIntegrated, header=header)
    ...
    def generateVideo(self):
        ...
        images = [img for img in os.listdir(
            imageFolder) if img.endswith(".png")]
    ...
    def runSimulation(self):
        ...
        print("Approx influx: {} customers / s --- Approx maximum outflux: {} customers / s".format(
            self.probNewCustomer, approxOutFlux))
        self.gui.update_output(
            "Approx influx: {} customers / s --- Approx maximum outflux: {} customers / s".format(self.probNewCustomer, approxOutFlux))
        ...
        customersHeadExit = 0
        emittingCustomers = 0
        # variable for plotting images to check that no blocks at exits - we start to worry when >50
        maxQueue = WEIRDQUEUELIMIT
        for i in range(self.maxSteps):
            ...
            for ind in customersExit[::-1]:
                ...
                stepStr = "step {} ({} customers, {} for exit): customer {} left with {} on shopping list, {} total time in store, {} exposure".format(
                    i, len(self.customers), customersHeadExit, ti, tx, ty, tz)
        ...
        # storePlot.showStore(step=self.stepNow)
        return storePlot
